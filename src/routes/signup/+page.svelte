<script lang="ts">
	...
	import { currentUser as cu } from '$lib/store';
	...
	const handleSignup = async () => {
		let data = { username, password, name, lastName };
		...
		if (result !== null) {
			...
			// if login request does not fail
			if (loginResult !== null) {
				...
				toast.success('Accounted created successful!, you have been logged in');
				console.log('Redirecting ...');
				setTimeout(() => {
					goto('/');
				}, 2000);
			} else {
				toast.info('Could not login user!');
			}

			// If account creation failed, display error message
		} else {
			toast.warning('Account was not created, user already exist!');
		}
	};
</script>
	...
	>
		...
		>
			...
			<header id="header-4a" class="flex items-center">
				<h3 class="flex-1 text-lg font-medium text-slate-700">
			...
			<div id="content-4a" class="flex-1">
				...
				<div class="text-xs float-right text-slate-400 my-2">
					<a
						href="/login"
